...
<CodePen className={styles["full-bleed"]} slugHash="GRyWvZQ" />

## Orthographic projection

If we want to provide *screen-space coordinates* (pixels) to our buffers rather than clip-space coordinates, we can also do that using matrices.
Typically, transforming screen-space coordinates to clip-space coordinates inside of a shader would look something like this:
...
Which is basically equivalent to three scale operations and one translation operation.
The matrix required for this transformation looks like this (where `w` and `h` are the width and the height of the canvas, respectively):
...
Now we can pass pixel values into our buffer instead of clip-space coordinates:
...
<CodePen className={styles["full-bleed"]} slugHash="KKZWELG" />

---

**IMPORTANT**: Note that matrices shown in this tutorial are in *column-major* order:
...
Because programming works in rows rather than columns, this appears backwards from what you're probably used to.
The math convention for matrices is to work in columns, so any matrices you find online will need to be *transposed* (flipped diagonally) in order to be used in your code.

---

**IMPORTANT**: The matrix class in Umbra is optimized for ease of use, not for performance.
If high performance is very important in your application, I highly recommend using a dedicated matrix math library such as [glMatrix](https://glmatrix.net/).
