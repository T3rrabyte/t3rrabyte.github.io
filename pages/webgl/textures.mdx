...
Note that, since `NEAREST` and `LINEAR` only ever use the largest mip, it is not necessary to generate a mipmap for textures that use those functions.
Additionally, only `NEAREST` and `LINEAR` may be used for `TEXTURE_MAG_FILTER`.
