...
Since `NEAREST` and `LINEAR` only ever read the largest mip, it is not necessary to generate a mipmap when using them for the minification filter.
...
Umbra will automatically generate a mipmap when necessary.
...
<CodePen slugHash="poLRQdd" className={styles["full-bleed"]} />

## Data textures

Instead of loading a texture from an image, texture data can also be supplied directly from the code.
The way the data is interpreted depends on the *internal format* that is chosen.
For example, `R8` takes one eight-bit number per *texel* (short for "texture element") and normalizes it to a fraction.
Depending on which internal format is chosen, the corresponding external format (just "format") and data type must also be chosen.

```js
const level = 0;
const internalFormat = gl.R8; // Internal format
const width = 3;
const height = 2;
const border = 0;
const format = gl.RED; // External format for R8
const type = gl.UNSIGNED_BYTE; // Data type for R8
...
In Umbra, the format and data type are decided automatically based on the internal format.

```js
texture.internalFormat = TextureFormat.R8;
```

**Textures will not update if their width is not a multiple of the unpack alignment.**
The unpack alignment defaults to four in order to remain backwards compatibility with WebGL 1.0, but it can be set to one, two, four, or eight.
The example above uses a width of three, so the unpack alignment must be one in order for the texture to render.
This must be done before the call to `texImage2D` (or whatever other method you're using to update the texture).

```js
gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1);
```

Umbra sets the unpack alignment automatically as necessary when updating textures.

<CodePen slugHash="gOeRyjr" className={styles["full-bleed"]} />
...
<CodePen slugHash="rNdwgjg" className={styles["full-bleed"]} />
