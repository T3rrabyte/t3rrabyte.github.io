...
They can be used to rendering to textures and *renderbuffers*, which are objects that contain images.

Renderbuffers differ from images in that they support more formats but can't be sampled. Renderbuffers
also support *multisampling*, which is a process for reducing *aliasing*, which is an effect that
reduces the quality of images that are under-sampled. This means that multisampling is a form of *antialiasing*.
...
import RenderingToTextures from "./RenderingToTextures.jsx";
