...
_Directional lighting_ (also called _diffuse lighting_) is light that comes uniformly from one direction. If the direction that the light is traveling and the direction that a surface is facing are both known, it is possible to determine the brightness of the light on that surface. This is equivalent to the cosine of the angle between the directions, which is equivalent to the dot product of the directions.
...
The brightness of a surface is equivalent to the cosine of the angle between that surface's normal and the direction from that surface to the light source. The direction to the light source is the opposite of the direction that the light is traveling. The cosine can be calculated by taking the dot product of the angles. For example:
...
_Point lighting_ is when light extends in every direction from a point. To emulate it, first pass the direction from the surface to the light source as a varying. For example:
...
The direction to the light can then be used in exactly the same way as with directional lighting. Point lighting is almost identical to directional lighting, except that the light direction is interpolated between fragments.
