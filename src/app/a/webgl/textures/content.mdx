...
A _sampler_ is a uniform that is used to sample a texture. The `texture` function is used to get the
_texel_ (texture element) from the given texture at the given _texture space_ coordinates, which represent
...
To pass a texture to a sampler uniform, it must be bound to a _texture unit_ that represents it. The
...
A _mipmap_ is a collection of _mips_, which are smaller versions of a texture that are used to rasterize
...
In order for a texture to be rasterized, it must be _texture complete_, meaning that it either only
...
A _texture atlas_ is a texture that contains multiple images. Using a texture atlas can simplify a
...
Textures will be malformed if their row alignment is not a multiple of their _unpack alignment_, which
...
_Projection mapping_ is the process of projecting an image. It can be accomplished by making another
