...
In this example, the lines `One`, `One`, `Two`, and `Two` will be printed, in that order. You might notice that `print` itself is a function. `print`, along with many other functions, are automatically included in every Python program from the _Python standard library_.
...
For example, if we want to make an image grayscale, we can do so by setting the red, green, and blue values of each pixel to the _luminance_ of that pixel. Luminance is calculated as follows:
