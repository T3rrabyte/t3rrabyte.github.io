...
import { Color, Program, Buffer, VAO, AttributeState, clearContext, resizeContext, UniformValue, Primitive } from "@lakuna/umbra.js";
...
export default function UmbraMultipleViews({ ...props }) {
	return AnimatedCanvas((canvas: HTMLCanvasElement) => {
		...
		return function render(now: number) {
			...
			fVao.draw({ "u_color": green, "u_matrix": tempMat as UniformValue });
			...
			wireframeCamVao.draw({ "u_matrix": tempMat as UniformValue }, Primitive.LINES);

			mat4.multiply(tempMat, tempMat, wireframeCubeMat);

			wireframeCubeVao.draw({ "u_matrix": tempMat as UniformValue }, Primitive.LINES);
		}
	}, props);
}
