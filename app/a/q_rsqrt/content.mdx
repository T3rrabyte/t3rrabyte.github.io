...
called the "*mantissa*", and the exponent of the $10$ ($4$) is just called the "*exponent*."

IEEE 754 floating-point values are stored in 32 bits. The first bit is the "*sign bit*." The sign bit
...
The next `8` bits encode the exponent. This number is right-aligned. Rather than using *two's complement*, which is a
...
- If the exponent field is all zeros, the number is "*denormalized*," which means that the exponent becomes $1-b$, where
