...
To translate an initial vector $(x_0,y_0,z_0)$ by $(x_t,y_t,z_t)$, resulting in a vector $(x_1,y_1,z_1)$:
...
To rotate an initial vector $(x_0,y_0,z_0)$ by $r$ radians around the $z$ axis (most common in two dimensions),
...
For the $(x_a,y_a,z_a)$ axis, where $l=\sqrt{{x_a}^2+{y_a}^2+{z_a}^2}$ and $t=1-c$:
...
To scale an initial vector $(x_0,y_0,z_0)$ by a scaling vector $(x_s,y_s,z_s)$, resulting in a vector $(x_1,y_1,z_1)$:
...
since that operation is just a scale and a rotation. To convert a clip space vector $(x_0,y_0,z_0,w_0)$ to a screen space vector
$(x_1,y_1,z_1,w_1)$, where $l$, $r$, $b$, $t$, $n$, and $f$ are the left, right, bottom, top, near, and far bounds of the frustum,
respectively:
...
In order to avoid implementing all of the algorithms described above, subsequent examples will use the
[glMatrix](https://glmatrix.net/) library to perform matrix math.
