...
*location*. Umbra does this step automatically when linking shader programs.
...
Umbra has a function that is equivalent to the block of code above:
...
data from `positionBuffer` in intervals of $2$. Umbra has an equivalent function that looks like this:
...
Umbra has an equivalent function to the code block above that looks like this:
...
Note that Umbra automatically uses the correct draw function, so no changes are necessary when adding indices
to an Umbra VAO.
