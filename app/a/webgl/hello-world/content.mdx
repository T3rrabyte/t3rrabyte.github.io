...
You may need to read the previous articles about [WebGL](/a/webgl/what-is), [attributes](/a/webgl/attributes),
...
If you aren't using Umbra, see the [introductory article](/a/webgl/what-is) for the "vanilla" way to do this.
...
Umbra automatically creates a map of variable names to their locations when it links a shader program, so no
...
If you aren't using Umbra, see the [attributes](/a/webgl/attributes) and [uniforms](/a/webgl/uniforms) articles for
...
needed. In Umbra, the following line of code creates a buffer.
...
If you aren't using Umbra, see the [attributes](/a/webgl/attributes) article for the "vanilla" way to do this.

### Vertex array objects

This program will only draw one shape (a triangle), so only one vertex array object (VAO) is needed. In Umbra, the following
...
Umbra provides a function called `clearContext` that clears all of the viewport's buffers to the provided values. It
