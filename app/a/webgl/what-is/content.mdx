...
A canvas can be created in HTML.
...
low-level, such as [TWGL](https://twgljs.org/) and [μGL](https://ugl.lakuna.pw/). μGL is my library, so
it is what will be used in this tutorial series.

μGL uses its own `Context` class instead of the WebGL API's provided `WebGL2RenderingContext` class.
