...
const oneLiners = [
	<span key={0}>{"Also check out "}<Link href="https://griff.pw/">{"griff.pw"}</Link>{"!"}</span>, // Griffon Hanson
	<span key={1}>{"Also check out "}<Link href="https://ty.business/">{"ty.business"}</Link>{"!"}</span>, // Ty Morrow
	<span key={2}>{"Also check out "}<Link href="https://www.xanycki.art/">{"xanycki.art"}</Link>{"!"}</span>, // Anonymous friend
	...
	<span key={14}>{"U U D D L R L R B A S"}</span>, // The Konami Code
	...
	<span key={44}><a href="https://discord.gg/sdcleague">{"SDC"}</a>{" champion."}</span>, // The Seasonal Draft Championship; League of Legends
